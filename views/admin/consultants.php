    ...
    <div class="container">
      ...
      <h1 class="mt-4 mb-3">Журнал работников консультантов
        <small>админка</small>
      </h1>
...
<p class="card-text">Описание сообщения</p>
		...
		<div class="card-body">
		  ...
		  <div class="form-group row">
		  <label for="name" class="col-4 col-form-label col-form-label-sm">Подразделение:</label>
		  <div class="col-8">
...
<table class="table table-sm table-hover " id="jform_">
  <thead class="thead-inverse ">
    <tr >
      ...
      <th width="140">Наименование</th>
	  <th width="140">Подразделение</th>
    </tr>
  </thead>
  <tbody>
  </tbody>
...
$(function(){
	...
	$('input[name="department"]').typeahead({
		...
		source:  function (query, process) {
			$.get('get_typeheaddepartment', {q: query}, function(result){
 ...
 informer = {
    ...
    change : function(){
	  ...
	  $.post('consultants_change', {data:JSON.stringify(form_.data)},
    function(f) {
      if (f.status == 'success') {
...
del : function(){
	...
	$.post('consultants_del', {data:JSON.stringify(form_.data)},
	function(f) {
	  if (f.status == 'success') {
...
add : function(){
 ...
 var $inputs = $('#form_ input[type="text"],#form_ input[type="hidden"], #form_ textarea,#form_ select');		 
	  ...
	  $.post('consultants_add', {data:JSON.stringify(form_.data)},
    function(f) {
      if (f.status == 'success') {
        informer.set('Выполнен', f.message, "success");
