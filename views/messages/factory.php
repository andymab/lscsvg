    ...
    <div class="container">
      ...
      <h1 class="mt-4 mb-3"><?=$this->model->title?> <?=$this->model->userdata['factory']?>
      </h1>

      <ul class="breadcrumb">
        <li ><a href="<?=URL?>">Главная</a></li>
        <li ><a href="<?=URL?>/messages/">Сообщения</a></li>
		<li ><a href="#"><?=$this->model->title?> <?=$this->model->userdata['factory']?>, пользователь <?=$this->model->userdata['uname']?></a></li>
      </ul>
...
<p class="card-text">Описание сообщения</p>
		...
		<div class="card-body">
		  ...
		  <label for="name" class="col-form-label col-form-label-sm">Вопрос:</label>
			<textarea class="form-control  form-control-sm" type="text" value="" name="question"></textarea>
			<!--        <p class="help-block">Example block-level help text here.</p> -->
		  </div>
		  
		  <div class="form-group row">
		  <label for="descr" class="col-4 col-form-label col-form-label-sm">Консультант:</label>
		  <div class="col-8">
...
$(function(){
	...
	$('input[name="consultant"]').typeahead({
		...
		source:  function (query, process) {
			$.get('/admin/get_typeheadCons', {q: query}, function(result){
	...
	$('input[name="status"]').typeahead({
		...
		source:  function (query, process) {
			$.get('/admin/get_typeheadStatus', {q: query,profile:'users_post'}, function(result){
 ...
 informer = {
	...
	init : function(){
		...
		{"data": "date_update",
		"mRender":function(date_update,type,full){
			if(type == 'display'){
                var d = new Date(parseInt(date_update * 1000));
    ...
    change : function(){
	  ...
	  $.post('/messages/message_change', {data:JSON.stringify(form_.data)},
    function(f) {
      if (f.status == 'success') {
...
del : function(){
	...
	$.post('/messages/message_del', {data:JSON.stringify(form_.data)},
	function(f) {
	  if (f.status == 'success') {
...
add : function(){
 ...
 var $inputs = $('#form_ input[type="text"],#form_ input[type="hidden"], #form_ textarea,#form_ select');		 
	...
	$.post('/messages/message_add', {data:JSON.stringify(form_.data)},
    function(f) {
      if (f.status == 'success') {
        informer.set('Выполнен', f.message, "success");
