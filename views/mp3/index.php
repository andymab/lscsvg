...
hr{
    ...
    <div class="container">
      ...
      <h1 class="mt-2">Маршруты 3
	  <?php if($this->model->contentEditable) :?>
	  <a class="btn btn-primary btn-sm " href="#" onclick="return addkontr()" role="button">Новый Контрагент</a>
      <?php endif;?>
	  
      <small class="pull-right mb-0" id="LastUid"></small></h1>
      <hr class="mt-1">
...
<div class="col-8">
<h5 class="alert-heading"> <div id="indextbl" hidden=""></div><div id="idkontr" hidden=""></div> <div data-item="name" <?=$this->model->contentEditable?>>ООО "Декамюр"</div> <strong>код :</strong> <div data-item="cod" <?=$this->model->contentEditable?>>355666</div>
<?php if($this->model->contentEditable) :?>
<a class="btn btn-primary btn-sm mb-4 pull-right" href="#" onclick="return savekontr()" role="button">Сохранить</a>
<?php endif?>

</h5><hr>

<div class="alert alert-danger" role="alert">
  <strong>Внимание !!! </strong> <div data-item="alert" <?=$this->model->contentEditable?>> ДЕЛО У ЮРИСТОВ!!!!!!!!!!!!!!!!</div>
</div>
...
<div class="tab-content mb-5" id="IndexContent">
 ...
 <ul class="list-group">
 <li class="list-group-item list-group-item-warning"><strong>Условия :</strong> <div data-item="condition0" <?=$this->model->contentEditable?>>&nbsp;</div></li>
 <li class="list-group-item list-group-item-warning"><strong> Лимит :</strong> <div data-item="limit" <?=$this->model->contentEditable?>>&nbsp; </div></li>
       <li class="list-group-item"><strong>Комментарий :</strong> <div data-item="comment" <?=$this->model->contentEditable?>>Директор - Ирина Владимировна Рончка-8-909-496-22-88.</div></li> 
     <li class="list-group-item list-group-item-success"><strong>Телефоны :</strong> <div data-item="phones" <?=$this->model->contentEditable?>>8-(878-2)- 26-32-51-раб.-1-ый магази; 25-01-77-2-ой магазин; 25-49-88-торг.зал 2-ого магаз</li>
  <li class="list-group-item"><strong>Контакты :</strong> <div data-item="address" <?=$this->model->contentEditable?>>
  ...
  <li class="list-group-item list-group-item-info"><strong>email :</strong> <div data-item="email"<?=$this->model->contentEditable?>>jemchug09@yandex.ru</div></li>
...
function events0_tableAddRow(){
	...
	$.post('<?=URL?>mp3/addevents',{id_mp3:index.row.id},function(f){
		if(f.status="success"){
			var row = {}; 
			row.id = f.data.id;
...
function loadevents0(){
	index.events0.clear().draw(true);
	 $.post('<?=URL?>mp3/load_events',{id_mp3:index.row.id},function(f){
		 if(f.status == 'success'){
...
function addkontr(){
 ...
 function savekontr(){
    ...
    $.post('<?=URL?>mp3/change_kontr',{data:row},function(f){
        if(f.status == 'success'){
            row.id = f.data.id;
            ...
            if(indextbl != -1) {
                    ...
                    tr0.addClass('selected').css({'background-color':'#a5e4a5'});
                
                (function(el){
                    setTimeout(function(el){el.css({'background-color':''});}, 1500, el);
                })(tr0);
            } else {
...
index.table = $('#index_table').DataTable({
		...
		$.post('<?=URL?>mp3/getLastUid',{id:index.row.user_id},function(f){
			if(f.status=='success'){
...
index.events0 = $('#events0_table').DataTable({
		...
		"mRender":function(v1,type,full){
			return '<div data-item="v1" <?=$this->model->contentEditable?> >'+v1+'</div>';
		},
		},
        {"data": "v2",
		"mRender":function(v2,type,full){
			return '<div data-item="v2" <?=$this->model->contentEditable?> >'+v2+'</div>';
		},},
        {"data": "v3",
		"mRender":function(v3,type,full){
			return '<div data-item="v3" <?=$this->model->contentEditable?> >'+v3+'</div>';
		},},
        {"data": "v4",
		"mRender":function(v4,type,full){
			return '<div data-item="v4" <?=$this->model->contentEditable?> >'+v4+'</div>';
		},},
        {"data": "v5",
		"mRender":function(v5,type,full){
			return '<div data-item="v5" <?=$this->model->contentEditable?> >'+v5+'</div>';
		},},
        {"data": "v6",
		"mRender":function(v6,type,full){
			return '<div data-item="v6" <?=$this->model->contentEditable?> >'+v6+'</div>';
		},},
        ],
		"ordering": false,
        ...
        $.post('<?=URL?>mp3/change_events/',{name:title,data:row},function(f){
            if(f.status=="success"){
                thattd.css({'background-color':'#a5e4a5'});
                (function(el){
                    setTimeout(function(el){el.css({'background-color':''});}, 1500, el);
                })(thattd);
            } else {
                thattd.css({'background-color':'#f7ae6e'});
            }
          },'json');
        }
        ...
        var thatTr = $(this);
        var row = index.events0.row(thatTr).data();
		$.post('<?=URL?>mp3/getLastUid',{id:row.uid},function(f){
			if(f.status=='success'){
